...
class EspelhoDANFE(FPDF):
    def header(self):
        # Quadro Emitente e Identificação DANFE
        ...
        self.set_xy(105, 24)
        self.cell(35, 4, 'Nº 000.000.000', 0, 1, 'C') # Nota 0
        self.set_x(105)
        self.cell(35, 4, 'Série 0', 0, 1, 'C') # Série 0

        # Quadro Chave de Acesso e Protocolo
...
def gerar_pdf(df_final, params):
    ...
    # --- QUADRO: CÁLCULO DO IMPOSTO (LOGICA FISCAL IMPORTAÇÃO) ---
    pdf.set_font('Arial', 'B', 7)
    pdf.cell(190, 5, 'CÁLCULO DO IMPOSTO', 1, 1, 'L')
    
    # Linha 1: Bases e Total Produtos (Soma Mercadoria + II + PIS + COFINS + Taxas)
    ...
    # Linha 2: Frete, Seguro, Outras Despesas (AFRMM), IPI, Total Nota
    pdf.set_font('Arial', '', 6)
    pdf.cell(38, 4, 'VALOR DO FRETE', 'LR', 0, 'L')
    pdf.cell(38, 4, 'VALOR DO SEGURO', 'LR', 0, 'L')
    pdf.cell(38, 4, 'OUTRAS DESPESAS', 'LR', 0, 'L') # AFRMM vai aqui
...
st.title("📜 ARCANUM")
st.write("Auditoria de Importação - Projeto Sentinela")
st.divider()
...
if arquivo_subido and taxa_cambio > 0:
    ...
    if col_vlr and col_qtd:
        ...
        with col1:
            ...
            st.download_button("📥 Baixar Espelho em Excel", buffer_xlsx.getvalue(), "espelho_conferencia.xlsx")
        with col2:
